...
There arises a question: if the Verifier was to be selected by the Client, on Client's full responsibility, could we then omit the Client's choice not to release the payment voluntarily when W, V agree? That way the Client effectively would lose the right to appeal to Arbitration, and his interests are represented by Verifier only. However, it might still be beneficial to keep separate the right of agents to appeal to Arbitration, so that the faulty Verifier that conspires with the Worker can be punished.
